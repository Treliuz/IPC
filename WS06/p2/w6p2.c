...
int main(void)
{
    ...
    do
    {
        ...
        else if (forecastInput == 1)
        {
            printf("====================================================\n");
            printf("Filter: All items\n");
            printf("Amount: $%.2lf\n", sum);
            //Calculating how long it would take to finance on a yearly and monthly basis
            yearsFinancing = sum /  (incomeInput * 12);
            monthsFinancing = ((sum / incomeInput) - (yearsFinancing * 12) + 0.5) ;
            printf("Forecast: %d years, %d months\n", yearsFinancing, monthsFinancing);

            for (int j = 0; j < wishlistInput; j++)
            {
                if (financeInput[j] == 'y')
                {
        ...
        else if (forecastInput == 2)
        {
            ...
            for (int j = 0; j < wishlistInput; j++)
            {
                if (prioInput[j] == prioFilter)
                {
                    prioCost += costsInput[j];
                }
                
            }
            
            printf("Amount: $%.2lf\n", prioCost);
            yearsFinancing = prioCost /  (incomeInput * 12);
            monthsFinancing = ((prioCost / incomeInput) - (yearsFinancing * 12) + 0.5);
            printf("Forecast: %d years, %d months\n", yearsFinancing, monthsFinancing);
            printf("\n====================================================\n");            
        }
    } while (flag == 1);
